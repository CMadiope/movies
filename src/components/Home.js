...
const Home = () => {
  ...
  const [trendingInput, setTrendingInput] = useState("all");

  const togglePopularMovies = () => {
    setTrendingInput("movie");
  };

  const togglePopularSeries = () => {
    setTrendingInput("tv");
    console.log("changed to series");
  };

  const fetchTrending = async () => {
    const { data } = await tmdb.get(`trending/${trendingInput}/day`);
      ...
      <div className=''>
        <div className='flex items-center gap-6'>
          ...
          <div className='flex gap-3'>
            <div
              className='flex items-center gap-2 bg-gray-500 py-px px-2 rounded-xl cursor-pointer'
              onClick={() => {
                setTrendingInput("movie");
              }}
            >
            ...
            <div>
              <div
                onClick={togglePopularSeries}
                className='flex items-center gap-2 bg-green-500 py-px px-2 rounded-xl cursor-pointer'
              >
